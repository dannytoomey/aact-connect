...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.search{
        ...
        let get_results = aact_connect::get_results(
            args.username.as_deref().unwrap(),
            args.password.as_deref().unwrap())
            .await?;
        println!("Storing results as dataframe...");
        let df = aact_connect::result_struct_to_polars(get_results)?;

        let threads_used = args.threads;
        
        println!("Adding additional results using {} threads...",threads_used);
        let add_results = aact_connect::add_results(
            args.username.as_deref().unwrap(),
            args.password.as_deref().unwrap(),
            df.clone(),
            threads_used.into())
        ...
        let add_df = aact_connect::add_struct_to_polars(add_results)?;
        let mut combined_df = df.left_join(&add_df,["nct_id"],["nct_id"])?;
        
        let mut date = format!("{}",chrono::offset::Local::now());
        date = date[0..10].to_string();
        let path = format!("additional_data/query_{}_additional_data_{}.csv",combined_df.shape().0,date);
